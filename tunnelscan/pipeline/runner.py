...
def run_tunnelscan(
    ...
    n_centroid_steps = 50 if fast_test else 500
    ...
    engine = QMMMEngine(atoms, qm_region, temperature=temperature, use_gpu=use_gpu)

    log.info("Step 4: Equilibration")
    eq_result = equilibrate(engine, temperature=temperature, fast_test=fast_test)

    log.info("Step 5: Classical production MD")
    classical_traj = run_production(
        engine, eq_result, donor_atom_index, acceptor_atom_index, hydrogen_atom_index,
        n_steps=n_classical_steps, dt=1.0
    )
    ...
    if run_convergence_check and not fast_test:
        ...
        if barrier_diff > 0.5:
            log.warning(f"Convergence check: N_beads=16 barrier differs by {barrier_diff:.3f} — consider more beads")
